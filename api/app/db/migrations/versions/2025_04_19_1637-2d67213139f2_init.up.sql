...
BEGIN
    ...
    INSERT INTO channels (
        name, guild, channel_group, is_general, "order"
    )
    VALUES (
...
CREATE TABLE IF NOT EXISTS channel_groups (
    ...
    guild    VARCHAR(26)                               NOT NULL,
    "order"  INTEGER                                   NOT NULL,

    CONSTRAINT channel_groups_pk PRIMARY KEY (id),
    CONSTRAINT channel_groups_name_guild_uq UNIQUE (name, guild),
    CONSTRAINT channel_groups_guild_fk FOREIGN KEY (guild) REFERENCES guilds (id) ON DELETE CASCADE,
    CHECK ( LENGTH(name) > 0 )
);
...
CREATE TABLE IF NOT EXISTS channels (
    ...
    CONSTRAINT channels_guild_fk FOREIGN KEY (guild) REFERENCES guilds (id) ON DELETE CASCADE,
    CONSTRAINT channels_channel_group_fk FOREIGN KEY (channel_group) REFERENCES channel_groups (id) ON DELETE CASCADE,
    CHECK ( LENGTH(name) > 0 )
);
...
CREATE TABLE user_guilds_order (
    ...
    CONSTRAINT user_guilds_order_user_fk FOREIGN KEY ("user") REFERENCES users (id) ON DELETE CASCADE,
    CONSTRAINT user_guilds_order_guild_fk FOREIGN KEY (guild) REFERENCES guilds (id) ON DELETE CASCADE
);
-- end users --
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    CONSTRAINT messages_author_fk FOREIGN KEY (author) REFERENCES users (id) ON DELETE SET NULL,
    CONSTRAINT messages_channel_fk FOREIGN KEY (channel) REFERENCES channels (id) ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS emojis (
    ...
    CONSTRAINT emojis_guild_fk FOREIGN KEY (guild) REFERENCES guilds (id) ON DELETE CASCADE,
    CONSTRAINT emojis_author_fk FOREIGN KEY (author) REFERENCES users (id) ON DELETE SET NULL,
